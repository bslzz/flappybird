    ...
    return {
      ...
      moveLeft: function(scrollDistance) {
        document.getElementById(this.pipeId).style.left =
          this.leftOffset - scrollDistance / this.movementRatio + 'px';
      },
      init: function(flip) {
        this.pipeId = 'pipe_' + getRandomInt(0, 12345);
        this.pipeSize = getRandomInt(100, 300);
  ...
  function makeBird() {
    return {
      ...
      addBird: function() {
        ...
        _newBird.style.filter = 'hue-rotate(' + getRandomInt(0, 170) + 'deg)';

        document.body.appendChild(_newBird);
      },
      fly: function(scrollDistance) {
        document.getElementById(this.birdId).style.left =
          this.birdSize + scrollDistance / this.movementRatio + 'px';

        if (!$('#' + this.birdId).is(':animated')) {
          $('#' + this.birdId)
            .animate({ top: '+=100px' }, this.bounceRate)
            .animate({ top: '-=100px' }, this.bounceRate);
        }
      },
      init: function() {
        this.topOffset = getRandomInt(20, 40);
        this.movementRatio = getRandomInt(10, 50);
        this.birdId = 'bird_' + getRandomInt(0, 12345);
        this.birdSize = getRandomInt(30, 100); // 500px
